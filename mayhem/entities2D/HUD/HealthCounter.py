...
class HealthCounter(Entity2D):
    def user_instantiate(self):
        self.score_label = pyglet.text.Label(text="Health: 0", batch=self.game_object.UI_batch)
        self.score_label.weight = "bold"
        self.update_size_and_position()

    def prepare_draw(self, delta):
        self.score_label.text = f"Health: {LocalPlayer.instance.health}"
        self.update_size_and_position()

    def update_size_and_position(self):
        self.score_label.font_size = Window.size.y / 30
        self.score_label.position = Vec3(15, Window.size.y - 55 - self.score_label.font_size, 0)
